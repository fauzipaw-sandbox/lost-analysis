...
# --- 1. SETUP KONEKSI SHAREPOINT ---
# Ganti dengan kredensial dan URL lo
SHAREPOINT_URL = "https://domainkantor.sharepoint.com/sites/NamaSiteLo"
SHAREPOINT_USER = "fauzi.ramdani@domainkantor.com"
SHAREPOINT_PWD = "PasswordLo123"
FILE_PATH = "/sites/NamaSiteLo/Shared Documents/Data_Availability.csv" 

@st.cache_data(ttl=3600) # Cache data 1 jam biar web gak lemot tiap di-refresh
def load_data_from_sharepoint():
    try:
        ...
        # DUMMY DATA buat testing UI kalau SharePoint belum connect
        return pd.DataFrame({
            'Site_ID': ['SITE-001', 'SITE-002', 'SITE-003', 'SITE-004'],
            'Child_Sites': ['SITE-002,SITE-003', '', '', ''], # Pisahkan pakai koma kalau di 1 kolom
            'Availability': [0.95, 0.90, 1.0, 0.85], # Persentase 0.0 - 1.0
            'Packet_Loss': [0.02, 0.05, 0.0, 0.08],
            'Actual_Revenue': [10000000, 5000000, 4000000, 8000000],
            'Actual_Payload': [500, 200, 150, 400] # dalam GB/TB
        })

# --- 2. LOGIC KALKULASI LOST REVENUE & PAYLOAD ---
def calculate_loss(df, site_id):
    # Cari data site utama (Parent)
    parent_data = df[df['Site_ID'] == site_id]
    
    if parent_data.empty:
        return None, None
        
    # Ambil list anakan (Asumsi kolom Child_Sites isinya comma-separated string)
    ...
    # Filter data Parent + Child
    all_related_sites = [site_id] + child_list
    filtered_df = df[df['Site_ID'].isin(all_related_sites)].copy()
    
    # Hitung Kalkulasi Lost
    # Rumus asumsi: Ideal = Actual / Availability -> Lost = Ideal - Actual
    # Tambahin proteksi pembagian dengan nol kalau Availability 0
    ...
    # Tandain mana yang Parent, mana yang Child
...
st.set_page_config(page_title="Network Loss Impact Dashboard", layout="wide")
...
if search_site:
    ...
    if impact_df is None or impact_df.empty:
        st.warning(f"Wah, Site ID {search_site} gak ketemu di data, Zi. Coba cek typo.")
    else:
